 ...
 *
 */
public class GRTS_MC_SEC extends TrajectoryCompressor {

    int count = 0;

    public GRTS_MC_SEC(DisCalculator dc) {
        super();
        this.setDc(dc);
    }

    public GPSLine[] compress() {
        return this.compressByGRTS_MC_SEC(this.iTrajectory, 0, this.iTrajectory.size() - 1);
    }
    ...
    private GPSLine[] compressByGRTS_MC_SEC(List<GPSPoint> iTraj, int iStart, int iEnd) {
        ...
        for (int i = iStart + 1; i <= iEnd; i++) {
            ...
            for (int n = Sstart + 1; n < i; n++) {
                ...
                if (n < lastCompressed) {
                    ...
                    disOfSi = DisCalculator.getDistanceOfP2P(iTraj.get(n), temp) + bThreshold;
                } else {
                    GPSPoint temp = DisCalculator.getEndPoint(iTraj.get(Sstart), iTraj.get(i), iTraj.get(n).time - iTraj.get(Sstart).time);
                    disOfSi = DisCalculator.getDistanceOfP2P(iTraj.get(n), temp);
                }
...
//                	System.out.println(i);
            ...
            if (i - lastCompressed == Bufferm - C) {
                for (int n = lastCompressed; n < i; n++) {
                    if (n == lastCompressed) {
                        GPSPoint temp = DisCalculator.getEndPoint(iTraj.get(Sstart), iTraj.get(i), iTraj.get(n).time - iTraj.get(Sstart).time);
                        double disOfSi = DisCalculator.getDistanceOfP2P(iTraj.get(n), temp) + bThreshold;
                        bThreshold = Math.max(bThreshold, disOfSi);
                    } else {
                        GPSPoint temp = DisCalculator.getEndPoint(iTraj.get(Sstart), iTraj.get(i), iTraj.get(n).time - iTraj.get(Sstart).time);
    ...
    public static void main(String[] args) throws IOException {
        ...
        TrajectoryCompressor tc = new GRTS_MC_SEC(sed);
...
//			tc.iTrajectory = tc.iTrajectory.subList(140, 144);
            ...
            System.out.println("	Compress By GRTS_MC_SEC to : (" + lines.length + ") lines");
